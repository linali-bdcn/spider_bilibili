...
class ExtraExtractor:
    ...
    def _get_base_info(video_url: str, cookie_str: str) -> dict:
        ...
        if pages:
            if p > len(pages): p = 1
            v_data['cid'] = pages[p-1]['cid']
            if len(pages) > 1:
                v_data['title'] = f"{v_data.get('title', bvid)}_P{p}_{pages[p-1].get('part', '')}"
                
        return v_data, headers, bvid
    ...
    def extract_subtitle(video_url: str, output_dir: str, output_format: str, cookie_str: str = "", progress_callback=None) -> dict:
        ...
        try:
            ...
            # 优先从 v2 API 获取，若为空则兜底从 view API 提取
            subtitles = sub_data.get('data', {}).get('subtitle', {}).get('subtitles') or []
            if not subtitles:
                subtitles = v_data.get('subtitle', {}).get('list') or []
                
            if not subtitles: return {"success": False, "message": "该视频没有字幕，或被风控拦截(请尝试更新Cookie)"}
            
            # 优先寻找 AI 生成字幕
            subtitle_url = next((s.get('subtitle_url') or s.get('url') for s in subtitles if s.get('lan') in ['ai-zh', 'ai']), None)
...
class BiliDownloader:
    ...
    def _show_splash(self):
        ...
        ttk.Label(frame, text="B站视频下载器 · 全能工具箱", font=("", 12, "bold")).pack(anchor=tk.W)
        ttk.Label(frame, text=f"版本: v{APP_VERSION}", foreground="gray").pack(anchor=tk.W, pady=(4, 12))

        ttk.Separator(frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=6)
        ttk.Label(frame, text="作者: linali_bdcn").pack(anchor=tk.W)
        ttk.Label(frame, text="源码: https://github.com/linali-bdcn/spider_bilibili", foreground="blue", cursor="hand2").pack(anchor=tk.W, pady=(2, 12))

        ttk.Separator(frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=6)
        ttk.Label(frame, text="Cookie 获取方式", font=("", 9, "bold")).pack(anchor=tk.W)
        for line in [
        ...
        ttk.Separator(frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=6)
        ttk.Label(frame, text="免责声明", font=("", 9, "bold")).pack(anchor=tk.W)
        for line in [
            "本工具仅供个人学习和研究使用，",
        ...
        ttk.Button(btn_frame, text="我知道了", command=splash.destroy, width=12).pack()
        ...
        splash.focus_force()
        splash.after(200, lambda: splash.bind("<FocusOut>", lambda e: splash.after(80, splash.destroy)))
        splash.wait_window()
    ...
    def _get_cookie_str_for_requests(self) -> str:
        """核心修复：将浏览器Cookie提取给 requests 使用，解决字幕被风控的问题"""
        method = self.cookie_method.get()
        if method == "file" and os.path.exists(self.cookie_file_var.get()):
            cookies = []
            try:
            ...
            try:
                # 借助 yt-dlp 的内部组件提取浏览器 Cookie 给 requests
                import yt_dlp.cookies
                cj = yt_dlp.cookies.extract_cookies_from_browser(self.browser_var.get())
                cookies = []
                for cookie in cj:

import requests
import re
"""from sort_vidoe import print_quality_dict as printq
from sort_vidoe import get_specific_quality as getq
from sort_vidoe import categorize_video_quality as sortq"""
from sort_vidoe import get_highest_quality_url as getq
# 定义URL和cookie
...
def match_urls(content):
    ...
    video_pattern3 = r'https://[^"]+?(?:(?:_x)?1-1-10\d{4})\.m4s[^"]*'
    video_pattern4 = r'https://[^"]+?/\d+-1-(?:10\d{4})\.m4s[^"]*'

    # 查找所有匹配
    audio_matches = re.findall(audio_pattern1, content)
    video_matches = re.findall(video_pattern4, content)
...
def save_f(audio_matches, video_matches, headers, name):
    ...
    if video_matches:
        get_video(video_matches, headers, name)
...
if __name__ == "__main__":
    #输入基本信息
    user_Agent = input("使用此程序需要自行截取信息（请自行设置请求头）\n请输入用户User-Agent:\n>>>")
    cookie = input("请输入用户Cookie:\n>>>")
    a = 1
    while(a):
        url = str(input("请输入视频url地址:(/q 退出)\n>>>"))
        if url == "/q": break
        headers = {
            # Referer 防盗链 告诉服务器你请求链接是从哪里跳转过来的
            # "Referer": "https://www.bilibili.com/video/BV1454y187Er/",
        ...
        #视频分类获取
        video_match = getq(video_matches)
        print(video_match)
        # 视频音频文件下载
        save_f(audio_matches, video_match, headers, title)
    # 单独调用时需要注意这是个列表
    """
